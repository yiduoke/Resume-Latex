%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Medium Length Professional CV
% LaTeX Template
% Version 2.0 (8/5/13)
%
% This template has been downloaded from:
% http://www.LaTeXTemplates.com
%
% Original author:
% Trey Hunner (http://www.treyhunner.com/)
%
% Important note:
% This template requires the resume.cls file to be in the same directory as the
% .tex file. The resume.cls file provides the resume style used for structuring the
% document.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\documentclass{resume} % Use the custom resume.cls style

\usepackage[left=0.75in,top=0.6in,right=0.75in,bottom=0.6in]{geometry} % Document margins

\name{Yidi Ke} % Your name
%\address{Website/Portfolio: http://yidi.me} % Your phone number and email
%\address{GitHub: https://github.com/rengokantai \\ Codepen: https://codepen.io/rengokantai} % Your secondary addess (optional)
%\address{Codepen: https://github.com/rengokantai}
%\address{Corona,NY \\(718) 501 0581 \\ yuriqiao@gmail.com } % Your phone number and email
\address{Queens, NY \\ (718) 501 0581 \\ yuriqiao@gmail.com} % Your address
%\address{GitHub: rengokantai} % Your secondary addess (optional)
%\address{Corona,NY \\(718) 501 0581 \\ yuriqiao@gmail.com } % Your phone number and email
%\address{GitHub: rengokantai }
%~$\textendash$~
%\address{ Git}
\begin{document}


%----------------------------------------------------------------------------------------
%	EDUCATION SECTION
%----------------------------------------------------------------------------------------

%\begin{rSection}{Personal}

%\end{rSection}
\begin{rSection}{Qualification}
As a creative and enthusiastic developer, I am passionate about modern, open web standards. I am interested in front-end and software development.Versed in OO languages such as Java. Familiar with version control system such as Git. Experienced with cloud computing and big data.
(U.S. Permanent Resident)

\end{rSection}


%    Before 10 08 2014
%\begin{rSection}{Technical Skills}

%\begin{tabular}{ @{} >{\bfseries}l @{\hspace{6ex}} l }
%Languages & Java, JSP, HTML5/Jade, CSS3/SASS/Compass, JavaScript(jQuery, D3.js, CoffeeScript)\\& Node.js, %AngularJS, MongoDB \\
%Tools & Git, Yeoman(Grunt/Gulp, Bower), Bootstrap Framework \\ & Photoshop, Illustrator

%Tools & Git, Yeoman(Grunt/Gulp, Bower), Bootstrap Framework \\ & Photoshop, Illustrator

%\end{tabular}

%\end{rSection}


%     After 10 08 2014
\begin{rSection}{Technical Skills}

\begin{tabular}{ @{} >{\bfseries}l @{\hspace{6ex}} l }
Languages& Java, Python \\
Web Development & SASS/Compass/Postcss,JavaScript(jQuery)\\& Ember/Node/Express, Twitter Bootstrap, SVG/snap.svg\\&
Karma/Mocha/Jasmine/Sinon/chai, Yeoman/Bower/gulp, Chrome DevTools/Fiddler \\
Databases & MySQL/MariaDB, PostgreSQL \\
DevOps and PAAS & Docker, Redis, Amazon Web Service, Azure, Hadoop\\
Tools & Git, Photoshop, Illustrator, Linux(Debian,Redhat), Vim, JetBrains IDE

\end{tabular}

\end{rSection}


%\begin{rSection}{Open-source community contribution(GitHub)}

%You don't know JS, A comprehensive JavaScript book. 28k stars\\
%Angular2-education(maintainer), A collection of Angular 2.0 books and tutorials. 3.4k stars\\
%Sails.js, A nodejs real-time framework. 14k stars\\
%Ember.js, Ambitious JS MVC Framework. 16k stars


%\end{rSection}














\begin{rSection}{Education}

{\bf Stony Brook University} \hfill {\em August 2012 - January 2015} \\ 
B.S. in Computer Science (GPA: 3.33)  \\
%Overall GPA: 3.35 \\
%Dean's List: Fall 2012 - Spring 2013 \\
%Courses: Advanced Programming in JAVA, Analysis of Algorithms \\
{\bf Laguardia Community College} \hfill {\em August 2009 - July 2012} \\ 
%(Freshman,Sophomore) \\
Major in Computer Science (GPA: 3.87) 
%Member of Math Society \\
%Overall GPA: 3.87
%Dean's List: Fall 2011 - Spring 2012 
\end{rSection}
%----------------------------------------------------------------------------------------
%	WORK EXPERIENCE SECTION
%----------------------------------------------------------------------------------------
\begin{rSection}{Experience}
\begin{rSubsection}{Youvisit}{\em April 2015 - July 2015 }{Full-Stack Web Developer Intern}{ New York, NY}
\item Built Virtual tour system for colleges and universities,developed UI widgets and functionality modules.
\item Wrote multiple scripts for version control and deployment. Python for version control, node.js and AWS lambda for cronjob
\item {\bf Technology in use: Sass, Three.js/D3.js/Highcharts, JavaScript, PHP, Node.js, AWS(EC2, Lambda), MySQL}

\item Live Demo http://www.stonybrook.edu/undergraduate-admissions/virtual-tour/
\end{rSubsection}
%\begin{rSubsection}{Tutor.com}{\em June 2014 - August 2014}{Computer Science Tutor}{New York, NY}
%\item TA of CSE114 under professor for two consecutive semesters (Fall 2013, Spring 2014).
%\item Tutoring Java for both beginning and intermediate levels.
%\end{rSubsection}
%\begin{rSubsection}{Tata Consultancy Services}{\em September 2015 - Current }{Software Enginneer}{New York, NY}
%\item Backend development using java.
%\item shell scripting under linux environment.
%\item Using hadoop and AWS to process large amount of data.
%\item Wrote gulp and webpack scripts to automate front-script building.
%\item Technology in use: Python/Django, celery, fabric, ansible, Ember/JavaScript
%\end{rSubsection}

%\begin{rSubsection}{Youvisit}{\em April 2015 - July 2015 }{Full-Stack Web Developer Intern}{ New York, NY}
%\item Built Virtual tour system for colleges and universities,developed UI widgets and functionality modules.
%\item Wrote multiple scripts for version control and deployment. Python for version control, node.js and AWS lambda for %cronjob
%\item Stack: Sass, Highcharts, JavaScript, PHP, Node.js, AWS(EC2, Lambda)
%\item Demo http://www.stonybrook.edu/undergraduate-admissions/virtual-tour/
%\end{rSubsection}

\begin{rSubsection}{Stony Brook University, Computer Science Department}{\em August 2013 - June 2014}{Teaching Assistant}{Stony Brook, NY}
\item TA of CSE114 under professor for two consecutive semesters (Fall 2013, Spring 2014).
\item Led students to learn JAVA and assisted the professor by correcting any mistakes they made.
\item Guided students, of all majors, to find their own solutions and joy in computer programming.
\end{rSubsection}

%------------------------------------------------


%------------------------------------------------

%\begin{rSubsection}{TinySoft}{January 2008 - April 2010}{Web Designer \& Developer}{Gainesville, GA}
%\item Vivamus PostgreSQL fermentum semper porta. Nunc diam velit PHP, adipiscing ut tristique vitae
%\item Maecenas convallis ullamcorper ultricies stylesheets.
%\item Quisque mi metus, unit tests CSS ornare sit amet fermentum et, tincidunt et orci.
%\item Curabitur venenatis pulvinar tellus gravida ornare. Sed et erat faucibus nunc euismod ultricies ut id
%\end{rSubsection}

\end{rSection}
%----------------------------------------------------------------------------------------
%	PROJECTS SECTION------           Yidi Ke
%----------------------------------------------------------------------------------------
\begin{rSection}{Some Personal Projects}

\item {\bf Birch-Library (JSP/JPA, Java, HTML/CSS/JavaScript, MySQL)} Implemented a book rental system allows users to rent, hold, rate and write a review for a book




\item {\bf Snapchat clone chat app (Swift)} Implemented a simple chat application similar to snapchat. Integrated with 3rd party parse API.



\item {\bf Ball Crush Saga(Java,XML)} Implemented a march -3 game derives from Facebook game Candy Crush Saga.



%\begin{rSubsection}{Node-based blogging system (Node.js, Express, monk)}{\em}{}{}
%\item Personal project. Implemented a blogging system.Users can post a blog, comment a blog..
%\item Node, express, Jade, MongoDB, monk, some other middlewares
%\end{rSubsection}


\item {\bf Flappy Pig (JavaScript)} Implemented Flappy Pig Game using vanilla JavaScript.

\end{rSection}

%----------------------------------------------------------------------------------------
%	ACHIEVEMENTS SECTION              YIdi K
%----------------------------------------------------------------------------------------
%\begin{rSection}{Achievements}
%{\bf AMATYC/SML Math Contest(NE Region)}, Rank 13 out of approx 700\hfill {\em January 2012}
%\end{rSection}



%----------------------------------------------------------------------------------------
%	TECHS SECTION                 YIdi Ke
%----------------------------------------------------------------------------------------
%\begin{rSection}{Technical Skills}

%\begin{tabular}{ @{} >{\bfseries}l @{\hspace{6ex}} l }
%Languages & Java, JSP, HTML5/Jade, CSS3/SASS/Compass, JavaScript(jQuery, D3.js, CoffeeScript)\\& Node.js, %AngularJS, MongoDB \\
%Tools & Git, Yeoman(Grunt/Gulp, Bower), Bootstrap Framework \\ & Photoshop, Illustrator

%Computer Languages & Prolog, Haskell, AWK, Erlang, Scheme, ML \\
%Protocols \& APIs & XML, JSON, SOAP, REST \\
%Databases & MySQL, PostgreSQL, Microsoft SQL \\
%Tools & SVN, Vim, Emacs
%\end{tabular}

%\end{rSection}


%\begin{rSection}{INTERESTS}
%Playing Basketball, Reading, Writing
%\end{rSection}
%----------------------------------------------------------------------------------------
%	REFS SECTION
%----------------------------------------------------------------------------------------
%\begin{rSection}{Reference}
%{\bf Paul Fodor}  \\
%Research Assistant Professor \\
%Computer Science Department \\
%Stony Brook University \\
%E-mail: pfodor@cs.stonybrook.edu \\
%Phone: 631-632-9820 
%\end{rSection}

%----------------------------------------------------------------------------------------
%	EXAMPLE SECTION
%----------------------------------------------------------------------------------------

%\begin{rSection}{Section Name}

%Section content\ldots

%\end{rSection}

%----------------------------------------------------------------------------------------

\end{document}
